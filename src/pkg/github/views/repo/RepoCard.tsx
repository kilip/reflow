...
import { patch, remove } from '../../utils/service'
  ...
  const handleConfirmation = async (confirmed: boolean, args: ConfirmArgs) => {
    ...
    if (ConfirmTypeEnum.delete === type && confirmed) {
      ...
      try {
        ...
        success(
          `Repository <strong>${repo.full_name}</strong> ${
    ...
    <div
      ...
      <InlineConfirmation<ConfirmArgs>
        ...
        <p className="text-center">
          ...
          <strong>{repo.archived ? 'unarchiving' : 'archiving'}</strong> this
          repository ?<br />
      ...
      <div
        className={classNames({
          'flex absolute top-0 bottom-0 w-full h-full rounded-md bg-slate-950 opacity-50':
            true,
          hidden: !loading,
        })}></div>
      <div
